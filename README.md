...
![치즈라면 쇼츠 영상 미리보기](./public/readme-assets/recipe-video-preview.png)

### 장면별 대본과 생성 이미지

![쇼츠 장면별 대본과 생성 이미지](./public/readme-assets/recipe-script-scenes.png)
...
- 레시피 등록, 조회, 수정, 삭제 기능
- 업로드 일정 관리 및 주간 캘린더
- Ollama, OpenAI, Gemini를 활용한 쇼츠 대본 자동 생성
- 대본에 맞는 장면별 이미지 4장 생성
- TTS를 활용한 한국어 내레이션 생성
- FFmpeg를 이용한 장면 이미지와 TTS 결합 및 쇼츠 영상 생성
- AI API 오류 발생 시 다른 제공자로 전환하는 fallback 처리
- API 입력값 검증 및 오류 처리
- 반응형 웹 화면과 채널 현황 대시보드
- Vercel 배포 및 환경변수 설정

### 가장 어려웠던 점 / 해결한 방법

가장 어려웠던 점은 생성된 이미지와 TTS를 하나의 영상으로 자연스럽게 결합하는 작업이었다. 처음에는 영상 전체에 첫 번째 이미지만 표시되는 문제가 발생했다.

원인을 분석한 결과 FFmpeg 영상 조립 과정에서 이미지 배열 중 첫 번째 이미지만 입력으로 사용하고 있었다. 이를 해결하기 위해 4개 이미지를 각각 FFmpeg 입력으로 전달하고, 대본의 `0~3초`, `3~8초`, `8~30초`, `30~40초` 구간 비율을 실제 TTS 길이에 맞게 계산했다. 이후 각 이미지를 순서대로 연결하여 음성 진행에 맞춰 장면이 전환되도록 구현했다.

또한 로컬에서는 Ollama가 정상적으로 동작했지만 Vercel 배포 환경에서는 OpenAI로 대본이 생성되는 문제가 있었다. 서버의 AI 제공자 선택 과정과 실제 API 응답을 확인하여 Ollama 호출이 실패하면 OpenAI fallback이 실행된다는 사실을 파악했다. 이를 통해 배포 환경에서는 환경변수 등록뿐 아니라 외부 서버의 공개 주소, 접근 가능 여부, 모델 설치 상태까지 함께 확인해야 한다는 것을 알게 되었다.

### 새롭게 배운 점 / 개선하고 싶은 점

이번 프로젝트를 진행하며 프론트엔드와 백엔드의 데이터 흐름뿐 아니라 여러 AI 서비스의 API를 하나의 기능으로 연결하는 방법을 배웠다. 구조화된 JSON 대본 생성, 이미지 생성, TTS, FFmpeg 영상 합성과 같이 서로 다른 형태의 결과물을 단계적으로 연결하는 경험을 할 수 있었다.

또한 환경변수는 단순히 등록하는 것으로 끝나는 것이 아니라 배포 서버에서 해당 주소에 실제로 접근할 수 있는지 확인해야 한다는 점을 배웠다. 오류 발생 시 다른 서비스로 전환하는 fallback 기능은 편리하지만 실제 장애 원인을 숨길 수 있으므로, 제공자별 오류 기록과 모니터링도 필요하다는 것을 알게 되었다.

앞으로는 다음 항목을 개선하고 싶다.

- 사용자 로그인 및 관리자 인증 추가
- AI API 호출 횟수 제한과 비용 보호 기능 적용
- JSON 파일 대신 PostgreSQL 또는 MySQL 사용
- Ollama 연결 실패 원인을 화면과 서버 로그에서 확인할 수 있도록 개선
- 장면별 TTS 생성으로 이미지 전환 시점을 더욱 정확하게 동기화
- 영상 자막, 전환 효과 및 배경음악 추가
- 실제 외부 API를 호출하지 않는 안전한 자동 테스트 환경 구축
